...
class SpecialPageFactoryTest extends \MediaWikiTestCase {
	...
	private function getFactory() {
		...
		$constraintChecker = $this->getMockBuilder( 'WikibaseQuality\ConstraintReport\ConstraintCheck\DelegatingConstraintChecker' )
			->disableOriginalConstructor()
			->getMock();
		$checkResultToViolationTranslator = $this->getMockBuilder( 'WikibaseQuality\ConstraintReport\Violations\CheckResultToViolationTranslator' )
			->disableOriginalConstructor()
			->getMock();
		$violationStore = $this->getMockForAbstractClass( 'WikibaseQuality\Violations\ViolationStore' );
